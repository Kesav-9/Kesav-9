...
def profile_page(request):
    ...
    customer_form=forms.BasicCustomerForm(request.POST,instance=request.user.customer)
    password_form=PasswordChangeForm(request.user)

    if request.method=="POST":

     if request.POST.get('action') == "upadte_profile":
        ...
        if user_form.is_valid() and customer_form.is_valid:
            ...
            return redirect(reverse('customer:profile'))
        elif  request.POST.get('action') == "upadte_password":
            ...
            return redirect(reverse('customer:profile'))
        elif  request.POST.get('action') == "upadte_phone":
...
def create_job_page(request):
    ...
    if request.method=="POST":
        if request.POST.get('step')=='1':
            ...
            try:
                    r=requests.get("https://maps.googleapis.com/maps/api/distancematrix/json?origns={}&destinations={}&mode=transit&key={}".format(
                    ...
                    create_job_page.save()

            except Exception as e:
                    print(e)
                    messages.error(request,"Unfortunately we don't support shipping at this distance")
